...
parser.add_argument('--cpu-arch', required=False, help="Manually specify CPU architecture, for when 'lscpu' does not report model name")
...
if not args.cpu_arch is None:
	for f in ["Times.mean.csv", "PAPI.mean.csv", "LoopNumIters.csv", "instruction-counts.csv"]:
		fp = os.path.join(tdir, f)
		df = pd.read_csv(fp, na_filter=False)
		df["CPU"] = args.cpu_arch
